...
//                         }
//                     },
//                     orders: lead.leadStage === "Converted" ? {
//                         create: {
//                             amount: coursePrice,
//                             orderNumber,
//                             paymentLink,
...
//         include: {
//             orders: { include: { product: { include: { productItems: { include: { course: true } } } }, course: { include: { systemForms: true } }, user: true } },
...
//             data: {
//                 course: { connect: { id: lead.orders[0]?.course?.id } },
//                 student: { connect: { id: lead.orders[0]?.user.id } },
//                 tester: { connect: { id: testers[Math.floor(Math.random() * testers.length)]?.id } },
//                 createdBy: { connect: { id: lead.assignee?.user.id } },
//                 writtenTest: { connect: { id: lead.orders[0]?.course?.systemForms.find(f => f.type === "PlacementTest")?.id } },
//                 zoomSessions: {
//                     create: {
