...
const FinalTestPage: NextPage = () => {
    ...
    const finalTestQuery = api.evaluationForm.getFinalTest.useQuery({ courseId })
    ...
    const handleSubmit = () => {
        ...
        submitMutation.mutate({
            answers,
            evaluationFormlId: finalTestQuery.data.finalTest.id,
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between flex-wrap gap-4">
                    <div className="flex items-center gap-4">
                        <ConceptTitle>{finalTestQuery.data.finalTest?.materialItem?.title} Assignment</ConceptTitle>
                        {
                            submission
                                ? (
                                    <SeverityPill color="info">
                                        {
                                            getEvalutaionStatus(
                                                finalTestQuery.data.finalTest.materialItem?.zoomSessions.find(session => session.materialItemId === finalTestQuery.data.finalTest?.materialItemId)?.sessionDate || new Date(),
                                                !!submission
                                            )
                                        }
                                    </SeverityPill>
                ...
                {!userQuery.data?.user?.zoomGroups.some(group => group.zoomSessions.some(session => session.materialItemId === finalTestQuery.data?.finalTest?.materialItemId))
                    ? (
