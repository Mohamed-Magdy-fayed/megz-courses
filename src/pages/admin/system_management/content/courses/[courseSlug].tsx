...
const CoursePage = () => {
    ...
    const onDelete = () => {
        if (!data?.course?.id) return toast({ variant: "destructive", title: "Error", description: "Invalid course ID!" })
        deleteMutation.mutate([data.course.id])
    }
    ...
    return (
        <AppLayout>
            <AlertModal
                ...
                onConfirm={onDelete}
                description="Warning, deleting a course may remove all related orders and operations!"
            />
                            ...
                            <FinalTestsTabContent
                                ...
                                finalTestSubmissions={data?.course.levels.flatMap(lvl => lvl.systemForms.flatMap(form => form.submissions.map(sub => ({ ...sub, systemForm: form }))))
                                    .map(({
                                        ...
                                        answers,
                                        student,
                                        oralQuestions: systemForm.oralTestQuestions,
