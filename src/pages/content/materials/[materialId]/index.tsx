...
const EditMaterialPage = () => {
  ...
  const { progress: downLoadProgress, downloadFile } = useFileDownload()
  ...
  const handleUpload = async (file: File) => {
    if (!file) return
    setUploading(true)
    const downloadURL = await uploadFile(file, `${pathQuery}/${file.name}`)
            ...
            <BreadcrumbList>
              {breadcrumbData.map((item, index) => (
                <div key={item.href} className="flex items-center gap-2">
                  <BreadcrumbItem>
                    <BreadcrumbLink className="cursor-pointer" onClick={() => setPathQuery(item.href)}>{item.label}</BreadcrumbLink>
                  </BreadcrumbItem>
        ...
        {loading ? (
          ...
          <>
                  ...
                  return (
                    <ContextMenu>
                      <ContextMenuTrigger asChild>
                        <Card className="[&>*]:p-2 justify-between flex flex-col" key={item.fullPath}>
                          <CardContent className="grid place-content-center">
                            <Image src={fileLogo} alt={item.fullPath.split(".")[1] || ""} height={100} width={100} />
                          </CardContent>
                          <CardFooter className="text-sm truncate">{item.name}</CardFooter>
                        </Card>
                      </ContextMenuTrigger>
                      <ContextMenuContent>
            ...
            <div className="flex flex-col gap-2 p-2">
              ...
              <div className="p-4 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                {prefixes.map(item => (
                  <Card key={item.fullPath} className="[&>*]:p-2 cursor-pointer" onClick={() => setPathQuery(item.fullPath)}>
                    <CardContent className="grid place-content-center">
